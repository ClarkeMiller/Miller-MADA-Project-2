...
Flu_Tab_wide <- ggplot () + geom_blank() + labs(title = "Reported Flu Cases for the 2023-2024 Seasn by WHO Region.")

ggsave("Flu_Tab_wide.png", plot = Flu_Tab_wide, device = "png")
...
# save to file
flu_table_file = here("assets", "flu_table_file.rds")
...
#These did not work worth a dang.
...
table_fileA = here("assets", "lmtableA.rds")
...
# save fit results table  
table_fileB = here("assets", "lmtableB.rds")
...
# save fit results table  
table_file1 = here("assets", "lmtable1.rds")
...
# save fit results table  
table_file2 = here("assets", "lmtable2.rds")
...
# save fit results table  
table_file3 = here("assets", "lmtable3.rds")
...
metrics_log1 <- logfitB1 %>% 
  predict(date_region2) %>% 
  bind_cols(date_region2) %>% 
  metrics(truth = EUR, estimate = .pred)


lmfitB2 <- lm(EUR ~ WPR + AMR + EMR, Region_offset) 
lmtableB1 <- broom::tidy(lmfitB1)
print(lmtableB1)

logfitB2 <- glm(EUR ~ WPR + AMR + EMR, data = Region_offset)
logtableB1 <- broom::tidy(logfitB1)
print(logtableB1)
