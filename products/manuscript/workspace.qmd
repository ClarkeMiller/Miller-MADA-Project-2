...
WHO_data_fluB <- WHO_data_flu %>% filter(WHOREGION == "AFR") %>% mutate (INF_AFR = INF_ALL)
WHO_data_fluB <- WHO_data_flu %>% filter(WHOREGION == "AMR") %>% mutate (INF_AMR = INF_ALL)
WHO_data_fluB <- WHO_data_flu %>% filter(WHOREGION == "EUR") %>% mutate (INF_EUR = INF_ALL)
WHO_data_fluB <- WHO_data_flu %>% filter(WHOREGION == "WPR") %>% mutate (INF_WPR = INF_ALL)

plot2A <- ggplot(WHO_data_fluB, aes(x = MMWR_WEEKSTARTDATE)) +
  geom_point(aes(y = INF_AFR), color = "red") +
  geom_line(aes(y = INF_AFR), color = "red") +
  geom_point(aes(y = INF_AMR), color = "blue") +
  geom_line(aes(y = INF_AMR), color = "blue") +
  geom_point(aes(y = INF_EUR), color = "green") +
  geom_line(aes(y = INF_EUR), color = "green") +
  geom_point(aes(y = INF_WPR), color = "magenta") +
  geom_line(aes(y = INF_WPR), color = "magenta") +
  labs(x = "DATE", y = "Total Reported Cases of FLU", title = "Scatterplot Flu Cases Per WHO Region for the 2023-2024 Flu Season") +
  theme_minimal()

plot2Asave = here("assets", "plot2A.png")
ggsave(plot2Asave, plot2A, width = 10, height = 6, dpi = 300)

#| label: fig-schematic
#| fig-cap: "Figure 6: World Flu Burden by Region Over the 2023-2024 Flu Season."
#| echo: FALSE
knitr::include_graphics(here("assets","plot2A.png"))
